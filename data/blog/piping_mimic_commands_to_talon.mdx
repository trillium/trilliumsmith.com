...
draft: false
summary: Quick and dirty terminal funciton for using Talon's mimic( )
---
...
This just mutes retun statements from the repl so that noise won't pollute the terminal as I'm working.

## What does M do

`M` is a helper function that automatically returns to the last used window. This makes it easy to run a short command eg `tab search, word github, south, lap`. In my repo that would execute in order:
...
```
 ...
 (Note: it's hard coded for mac at the moment but mimicing `command tab` so if that doesn't switch to your last window in your OS it will need updating).

## When is this actually a terrible choice to test something

Funnily enough, I had thought that using `mimic` to test if pieces of my Talon setup were functioning properly was totally fine. Turns out no, `mimic` is not the same as actually saying a whole phrase in Talon. This is important when testing commands from other sources like `talon_gaze_ocr`, which depends on a phrase being said to generate timestamp data. `mimic` does not gererate that data.