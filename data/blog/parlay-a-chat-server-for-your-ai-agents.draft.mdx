...
The tool itself depends on dictation engines that are local to the devices we own; it does not ship its own dictation engine. We can alias commands together, so for exmaple,
...
{/* Working notes — not blog content, resolve before publish:

Missing content:
- Separation of concerns diagram (flow now answered in prose above).
- Voice path diagram.
- Watch channels: how they work, how to subscribe, what the JSONL files look like.
- Beads: how they're created, how the done signal works in practice.
- herdr-web setup: step-by-step for getting the voice path working.
- Quickstart: more detail on setting PARLAY_DATA_DIR and PAI_DIR correctly.
- Honest caveats: more specifics on what's rough and what's not.
- Closing: stronger call to action.

Features to ship/merge:
- Voice path: stabilize and document herdr-web fork integration.
- Watch channels: document subscription and JSONL format.
- Beads: ship event trigger for bead updates/closes.
- Chat panel: release as side panel in `packages/client/`.
- Command list: document how to define commands and aliases.
- Quickstart: write detailed setup guide.
- Examples: add more worked examples.
- Mermaid diagrams: add rendering support to the site (or convert to static images).

QUESTION (captain): Should the voice dictation details (herdr-web setup, command definitions, voice path diagram) go in a separate post, or stay here? */}
