...
<Mermaid chart={`graph TD
    Phone[Phone - Dictation] -->|Voice Input| HerdrWeb[herdr-web - My Fork]
    HerdrWeb -->|Wraps Input| Herdr[herdr - Terminal Multiplexer]
    Herdr -->|Sends Commands| ParlayServer[Parlay - Command Server]
    ParlayServer -->|Routes to Agent| Agent[AI Agent]
    Agent -->|Replies| ParlayServer
    ParlayServer -->|SSE Stream| HerdrWeb
    HerdrWeb -->|Displays| Phone
    Tailscale[Tailscale] -.->|Private Network| Phone
    Tailscale -.->|Private Network| ParlayServer

    style Phone fill:#e1f5fe
    style HerdrWeb fill:#fff3e0
    style Herdr fill:#fff3e0
    style ParlayServer fill:#e8f5e9
    style Agent fill:#f3e5f5
    style Tailscale fill:#fce4ec
`} />

### What each part does

<Mermaid chart={`graph LR
    Phone[Phone] -->|Dictates commands| HerdrWeb[herdr-web]
    HerdrWeb -->|Wraps input box| Herdr[herdr]
    Herdr -->|Terminal I/O| Parlay[Parlay Server]
    Parlay -->|Evaluates commands| Agent[Agent Stream]

    subgraph Phone Layer
        Phone
    end
    ...
    subgraph Parlay Server
        Parlay
        SSE[SSE Stream]
        JSONL[JSONL Event Files]
    end
...
- [The Story](#the-story)
- [What Parlay is (how it works)](#what-parlay-is)
- [Voice-first input](#voice-first-input)
- [How to use this](#how-to-use-this)
- [The message bus](#the-message-bus)
- [Event-driven spawning](#event-driven-spawning)
